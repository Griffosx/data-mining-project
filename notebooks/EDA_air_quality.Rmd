...
for (var in response_vars) {
    p <- ggplot(
        ...
        scale_y_continuous(limits = quantile(data_air_quality[[var]], c(0.05, 0.95), na.rm=TRUE)) +
        labs(title = var)
...
for (city in city_name_map$air_quality) {
    ...
    for (var in response_vars) {
        ...
        model <- lm(data_city[[var]] ~ ., data = data_city[, feature_vars])
        # model <- lm(log(data_city[[var]] + 1) ~ ., data = data_city[, feature_vars])
