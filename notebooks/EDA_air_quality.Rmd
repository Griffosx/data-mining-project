...
if (DOWNLOAD_FILES) {
    ...
    for (i1 in seq_along(input_dirs$name)) {
        ...
        for (i2 in seq_along(input_files$name)) {
            ...
            cat(paste("Downloading", source_path, "to", destination_path, "\n"))
...
# % of NA by column and city, rounded to 3 decimal places
# sapply(data_air_quality, function(x) {
#     sapply(split(x, data_air_quality$City), function(y) round(sum(is.na(y)) / length(y), 3))
# })
```
...
for (var in response_vars) {
    n_bins <- min(length(unique(data_air_quality[[var]])), 101)
...
for (var in response_vars) {
    p <- ggplot(
            data_air_quality,
            aes_string(x = "factor(month(Datetime))", y = var, fill = "City")
        ) +
        geom_boxplot() +
        labs(title = var)
...
```{r}
feature_vars <- c(
    "sunHour", "uvIndex...7", "moon_illumination",
...
feature_vars <- c(
    ...
    "humidity", "precipMM", "pressure", "tempC", "visibility", "windspeedKmph", "day_of_week",
    "month_of_year", "hour_cos", "hour_sin", "winddir_cos", "winddir_sin"
)
...
for (city in city_name_map$air_quality) {
    ...
    for (var in response_vars) {
        # cat(paste("Fitting model for", var))

        model_formula <- paste(var, "~", paste(feature_vars, collapse = " + "))
        model <- lm(model_formula, data = data_city)
...
ggplot(feature_importance, aes(x = estimate, y = feature, fill = response_var)) +
    geom_bar(stat = "identity") +
    facet_wrap(~ city) +
